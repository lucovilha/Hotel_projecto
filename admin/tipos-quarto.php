<?php 
 ...
 if ($_SERVER['REQUEST_METHOD'] === 'POST') { 
     ...
     if ($acao === 'criar') { 
         ...
         if (empty($nome) || !$cap_base || !$cap_max || !$preco) { 
             ...
             $erro = 'A capacidade máxima não pode ser menor que a base.'; 
         } else { 
 ...
 $tipos = mysqli_query($conn, "SELECT * FROM tipos_quarto ORDER BY nome"); 
 ?> 
 <!DOCTYPE html> 
 <html lang="pt"> 
 <head> 
 ...
 <body> 
     ...
     <main> 
         ...
         <form method="POST"> 
             ...
             <label>Capacidade Base <input type="number" name="capacidade_base" min="1" required></label> 
             <label>Capacidade Máxima <input type="number" name="capacidade_maxima" min="1" required></label> 
             <label>Preço/noite (€) <input type="number" name="preco_diaria" step="0.01" min="0" required></label> 
             <label>Suplemento hóspede extra (€) <input type="number" name="preco_hospede_extra" step="0.01" min="0" value="0"></label> 
             <label>Pequeno-almoço por hóspede (€) <input type="number" name="preco_pequeno_almoco" step="0.01" min="0" value="0"></label> 
